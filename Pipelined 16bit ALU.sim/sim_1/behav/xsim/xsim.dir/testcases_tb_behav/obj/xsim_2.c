...
extern "C" {
	...
	extern void alu_model(char arg0, short arg1, short arg2, short* arg3, char* arg4, char* arg5) ;
}


extern "C" {
	void Dpi_wrapper_alu_model(char *GlobalDP, char *IP, char *SP)
	{
DPISetMode(1) ; // Function chain mode : with or without context 
		...
		(*((void**)(SP + 184))) = funcContext;
		DPISetCurrentContext(*(void**)(SP + 184));
		ptrToSP = (char*)((int*)ptrToSP + 1); 
		...
		ptrToSP = SP + 360 ; 
		char cObj0;
		DPIMemsetSvToDpi( (char*)(&cObj0), ptrToSP, 1, 1 );
		ptrToSP = ptrToSP + 8; 

		ptrToSP = SP + 552 ; 
		short cObj1;
		DPIMemsetSvToDpi( (char*)(&cObj1), ptrToSP, 2, 1 );
		ptrToSP = ptrToSP + 8; 

		ptrToSP = SP + 744 ; 
		short cObj2;
		DPIMemsetSvToDpi( (char*)(&cObj2), ptrToSP, 2, 1 );
		ptrToSP = ptrToSP + 8; 

		ptrToSP = SP + 936 ; 
		short cObj3;
		DPIMemsetSvToDpi( (char*)(&cObj3), ptrToSP, 2, 1 );
		ptrToSP = ptrToSP + 8; 

		ptrToSP = SP + 1128 ; 
		char cObj4;
		DPIMemsetSvToDpi( (char*)(&cObj4), ptrToSP, 1, 1 );
		ptrToSP = ptrToSP + 8; 

		ptrToSP = SP + 1320 ; 
		char cObj5;
		DPIMemsetSvToDpi( (char*)(&cObj5), ptrToSP, 1, 1 );
		ptrToSP = ptrToSP + 8; 

		    /******* Done Conversion of SV types to DPI-C Types *******/ 
		    /******* Call the DPI-C function *******/ 
		DPISetFuncName("alu_model");
		fflush(stdout); fflush(stderr);
		alu_model(cObj0, cObj1, cObj2, &cObj3, &cObj4, &cObj5);
		...
		ptrToSP = SP + 936 ; 
		DPIMemsetDpiToSv( ptrToSP, (char*)(&cObj3), 2, 1 );
		ptrToSP = ptrToSP + 8; 
		ptrToSP = SP + 1128 ; 
		DPIMemsetDpiToSv( ptrToSP, (char*)(&cObj4), 1, 1 );
		ptrToSP = ptrToSP + 8; 
		ptrToSP = SP + 1320 ; 
		DPIMemsetDpiToSv( ptrToSP, (char*)(&cObj5), 1, 1 );
		ptrToSP = ptrToSP + 8; 
